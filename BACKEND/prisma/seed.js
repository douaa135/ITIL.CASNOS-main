...
const statutsRfc = [
  { code_metier: 'STAT-RFC-PAP', code_statut: 'PRE_APPROUVEE',  libelle: 'Pré-approuvee',   description: 'RFC pré-approuvée',                 contexte: 'RFC' },
...
// ============================================================
// NOTIFICATIONS — CORRECTION : expediteurEmail par notif
// ============================================================
const notificationsRaw = [
  {
    code_metier:       'NOT-SEED-001',
    expediteurEmail:   'change.manager@casnos.dz', // c'est lui qui a approuvé
    destinataireEmail: 'demandeur@casnos.dz',
    message:           'Votre RFC-SEED-001 a été approuvée par le CAB. Un changement CHG-SEED-001 a été créé.',
    objet:             'RFC approuvée — RFC-SEED-001',
    type_notif:        'IN_APP',
    lue:               true,
    rfcCode:           'RFC-SEED-001',
    changementCode:    null,
  ...
  {
    code_metier:       'NOT-SEED-002',
    expediteurEmail:   'systeme@itil.internal',    // création automatique du changement
    destinataireEmail: 'change.manager@casnos.dz',
    message:           'Un nouveau changement CHG-SEED-001 vous a été assigné. Veuillez prendre en charge la planification.',
    objet:             'Nouveau changement assigné — CHG-SEED-001',
    type_notif:        'EMAIL',
    lue:               false,
    rfcCode:           null,
    changementCode:    'CHG-SEED-001',
  ...
  {
    code_metier:       'NOT-SEED-003',
    expediteurEmail:   'change.manager@casnos.dz', // c'est lui qui a rejeté
    destinataireEmail: 'demandeur@casnos.dz',
    message:           'Votre RFC-SEED-003 a été rejetée par le CAB. Motif : Budget non alloué pour ce trimestre.',
    objet:             'RFC rejetée — RFC-SEED-003',
    type_notif:        'IN_APP',
    lue:               true,
    rfcCode:           'RFC-SEED-003',
    changementCode:    null,
  },
  {
    code_metier:       'NOT-SEED-004',
    expediteurEmail:   'systeme@itil.internal',    // convocation automatique
    destinataireEmail: 'cab@casnos.dz',
    message:           'Réunion CAB planifiée le 20/01/2025 à 09h00. Ordre du jour : RFC-SEED-001 et RFC-SEED-003.',
    objet:             'Convocation CAB — REU-SEED-001',
    type_notif:        'EMAIL',
    lue:               true,
    rfcCode:           null,
    changementCode:    null,
  ...
  {
    code_metier:       'NOT-SEED-005',
    expediteurEmail:   'systeme@itil.internal',    // soumission au CCU automatique
    destinataireEmail: 'servicedesk@casnos.dz',
    message:           'Votre RFC-SEED-002 (CVE-2025-1234) a été soumise au CCU. Traitement en urgence en cours.',
    objet:             'RFC urgente en traitement — RFC-SEED-002',
    type_notif:        'SMS',
    lue:               false,
    rfcCode:           'RFC-SEED-002',
    changementCode:    null,
  ...
  {
    code_metier:       'NOT-SEED-006',
    expediteurEmail:   'systeme@itil.internal',    // clôture automatique
    destinataireEmail: 'rachid.boukhari@casnos.dz',
    message:           'Le changement CHG-SEED-004 a été clôturé avec succès. PIR disponible.',
    objet:             'Changement clôturé — CHG-SEED-004',
    type_notif:        'IN_APP',
    lue:               false,
    rfcCode:           null,
    changementCode:    'CHG-SEED-004',
...
async function main() {
  ...
  // ── Utilisateur SYSTÈME (acteur automatique) ─────────────────
  console.log('   🤖 Création utilisateur SYSTÈME...');
  ...
  const systemeUser = await prisma.utilisateur.upsert({
    ...
    create: {
      ...
      mot_passe:      await bcrypt.hash(crypto.randomUUID(), 12),
      actif:          true,
    },
  });

  await prisma.userRole.upsert({
    where:  { id_user_id_role: { id_user: systemeUser.id_user, id_role: roleSysteme.id_role } },
    update: {},
    create: { id_user: systemeUser.id_user, id_role: roleSysteme.id_role },
  });

  ids.users['systeme@itil.internal'] = systemeUser.id_user; // disponible pour les notifs
  console.log('   🤖 systeme@itil.internal → SYSTEME');
  ...
  for (const n of notificationsRaw) {
    const id_expediteur   = ids.users[n.expediteurEmail];   // ← expéditeur réel par notif
    ...
    const notification = await prisma.notification.upsert({
      ...
      update: { message: n.message, objet: n.objet },
      create: {
    ...
    await prisma.userNotification.upsert({
      where:  { id_notif_id_user: { id_notif: notification.id_notif, id_user: id_destinataire } },
      update: { lue: n.lue || false, date_lecture: n.lue ? new Date() : null },
      create: { id_notif: notification.id_notif, id_user: id_destinataire, lue: n.lue || false, date_lecture: n.lue ? new Date() : null },
    });
  }
  ...
  for (const b of blackoutsInitiaux) {
    try {
      await prisma.blackoutPeriod.upsert({
        ...
        update: { libelle: b.libelle, date_debut: b.date_debut, date_fin: b.date_fin, recurrent: b.recurrent, description: b.description, actif: true },
        create: { code_metier: b.code_metier, libelle: b.libelle, type: b.type, date_debut: b.date_debut, date_fin: b.date_fin, recurrent: b.recurrent, description: b.description, actif: true },
      });
  ...
  console.log(`   ${blkCreated} blackouts créés/mis à jour, ${blkSkipped} erreurs`);
  ...
  console.log(`   Rôles               : ${roles.length + 1} (dont SYSTEME)`);
  ...
  console.log(`   Utilisateurs        : ${usersRaw.length + 1} (dont SYSTEME)`);
  ...
  console.log(`   Blackouts           : ${blackoutsInitiaux.length}`);
  console.log('═'.repeat(62));
  ...
  console.log(`   ${'systeme@itil.internal'.padEnd(42)} SYSTEME  ← mot de passe aléatoire`);
}
