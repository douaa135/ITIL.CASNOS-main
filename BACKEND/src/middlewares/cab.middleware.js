...
const TYPES_CAB_VALIDES     = ['NORMAL', 'URGENT'];
 ...
 */
/**
 * checkUserIsCabMembre  ← REMPLACE l'ancienne version dans cab.middleware.js
 *
 * ✅ Accepte : membres permanents du CAB (membreCab) OU participants à la réunion
 * ❌ Bloque  : utilisateurs totalement étrangers à la réunion
 *
 * Doit être placé APRÈS checkReunionExists et checkUserExists.
 */
const checkUserIsCabMembre = async (req, res, next) => {
  try {
    ...
    // 1. Vérifier s'il est membre permanent du CAB
    ...
    // 2. ✅ NOUVEAU — vérifier s'il est participant à cette réunion spécifique
    const participant = await prisma.participant.findUnique({
      where: { id_reunion_id_user: { id_reunion, id_user } },
    });

    if (participant) {
      req.membreCab = { id_cab, id_user, role: 'PARTICIPANT' }; // rôle virtuel
      return next();
    }

    // 3. Ni membre ni participant → refus
    return R.forbidden(
      res,
      'Cet utilisateur n\'est ni membre de ce CAB ni participant à cette réunion.'
    );
  } catch (err) {
