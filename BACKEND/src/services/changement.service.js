...
const updateChangement = async (id, data, id_user = null) => {
  ...
  if (statut === 'EN_ECHEC') {
    ...
    await prisma.changement.update({
      where: { id_changement: id },
      data: {
        ...(date_debut !== undefined && { date_debut: date_debut ? new Date(date_debut) : null }),
        ...(date_fin_prevu !== undefined && { date_fin_prevu: date_fin_prevu ? new Date(date_fin_prevu) : null }),
      },
    });
    return getChangementById(id);
  }
