...
async function getAuditTrailByRfc(id_rfc) {
  try {
    const [changements, rfcData] = await Promise.all([
      ...
      prisma.rfc.findUnique({
        where:  { id_rfc },
        select: {
          rfcReunions:      { select: { id_reunion: true } },
          votesCab:         { select: { id_vote: true } },
          decisionsCab:     { select: { id_decision: true } },
          commentaires:     { select: { id_commentaire: true } },
          piecesJointes:    { select: { id_piece: true } },
          evaluationRisque: { select: { id_evaluation: true } },
        },
      }),
    ]);
    ...
    if (rfcData) {
      rfcData.rfcReunions.forEach(r     => entityIds.push(r.id_reunion));
      rfcData.votesCab.forEach(v        => entityIds.push(v.id_vote));
      rfcData.decisionsCab.forEach(d    => entityIds.push(d.id_decision));
      rfcData.commentaires.forEach(c    => entityIds.push(c.id_commentaire));
      rfcData.piecesJointes.forEach(p   => entityIds.push(p.id_piece));
      if (rfcData.evaluationRisque)
        entityIds.push(rfcData.evaluationRisque.id_evaluation);
    }

    return prisma.auditLog.findMany({
      where:   { entite_id: { in: [...new Set(entityIds)] } },
