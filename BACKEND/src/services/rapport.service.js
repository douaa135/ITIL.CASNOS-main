...
const RAPPORT_SELECT = {
  id_rapport:      true, code_metier: true, titre_rapport: true,
  type_rapport:    true, contenu_rapport: true, date_generation: true,
  id_rfc:          true,
...
async function createRapport(id_rfc, data) {
  ...
  return prisma.rapport.create({
    data: { code_metier: codeRapport(), titre_rapport: titre_rapport.trim(), type_rapport, contenu_rapport, id_rfc },
    select: RAPPORT_SELECT,
  });
}

async function getRapportsByRfc(id_rfc) {
  return prisma.rapport.findMany({ where: { id_rfc }, orderBy: { date_generation: 'desc' }, select: RAPPORT_SELECT });
}
...
async function getFullRfcReport(id_rfc) {
  ...
  // ── 3. Réunions CAB liées à cette RFC ────────────────────────
    ...
    include: {
      reunion: {
        include: {
          ...
          participants: {
            select: { utilisateur: { select: { nom_user: true, prenom_user: true, email_user: true } } },
          },
          votesCab: {
            where: { id_rfc },
            include: {
              utilisateur: { select: { nom_user: true, prenom_user: true, email_user: true } },
            },
          },
          decisionsCab: {
            where: { id_rfc },
  ...
  return {
    ...
    rfc: {
      ...
      date_creation:   rfc.date_creation,
      date_modification: rfc.date_modification ?? null,
      date_souhaitee:  rfc.date_souhaitee ?? null,
      date_cloture:    rfc.date_cloture ?? null,
      urgence:         rfc.urgence,
      statut:          rfc.statut,
      priorite:        rfc.priorite,
      type:            rfc.typeRfc,
      demandeur:       rfc.demandeur,
    },
    ...
    evaluation_risque: rfc.evaluationRisque
      ? {
          impacte:         rfc.evaluationRisque.impacte,
          probabilite:     rfc.evaluationRisque.probabilite,
          score_risque:    rfc.evaluationRisque.score_risque,
          niveau:          _niveauRisque(rfc.evaluationRisque.score_risque),
          description:     rfc.evaluationRisque.description,
          date_evaluation: rfc.evaluationRisque.date_evaluation,
        }
      : null,

    commentaires: rfc.commentaires.map(c => ({
      auteur:           `${c.auteur.prenom_user} ${c.auteur.nom_user}`,
      email:            c.auteur.email_user,
      contenu:          c.contenu,
    ...
    pieces_jointes: rfc.piecesJointes.map(p => ({
      ...
      taille_piece: p.taille_piece?.toString() ?? null,
    ...
    changements: changements.map(chg => ({
      id_changement:    chg.id_changement,
      code_changement:  chg.code_changement,
      statut:           chg.statut,
      date_creation:    chg.date_creation,
      date_debut:       chg.date_debut ?? null,
      date_fin_prevu:   chg.date_fin_prevu ?? null,
      date_fin_reelle:  chg.date_fin_reelle ?? null,
      ...
      pir: chg.pir
        ? {
            date_pir:             chg.pir.date_pir ?? null,
            description:          chg.pir.description ?? null,
            conformite_objectifs: chg.pir.conformite_objectifs,
            conformite_delais:    chg.pir.conformite_delais,
          }
        : null,

      tests: chg.tests.map(t => ({
        critere_test: t.critere_test,
        resultat:     t.resultat,
        date_test:    t.date_test ?? null,
      ...
      taches: chg.taches.map(t => ({
        ...
        journaux:      t.journaux.map(j => ({
          titre:       j.titre_journal ?? null,
    ...
    reunions_cab: rfcReunions.map(r => ({
      code_metier:  r.reunion.code_metier,
      type_cab:     r.reunion.cab.type_cab,
      date_reunion: r.reunion.date_reunion,
      heure_debut:  r.reunion.heure_debut ?? null,
      heure_fin:    r.reunion.heure_fin   ?? null,
      ordre_jour:   r.reunion.ordre_jour  ?? null,
      ...
      votes: r.reunion.votesCab.map(v => ({
        ...
        email:       v.utilisateur.email_user,
        valeur_vote: v.valeur_vote,
        date_vote:   v.date_vote,
      })),
      decisions: r.reunion.decisionsCab.map(d => ({
        decision:      d.decision,
        motif:         d.motif ?? null,
    ...
    audit_trail: auditTrail.map(log => ({
      ...
      acteur:       log.utilisateur
        ...
        : 'Système',
      avant:        log.ancienne_val ?? null,
      apres:        log.nouvelle_val ?? null,
    })),
  };
}
...
function _computeStats(rfc, changements, auditTrail) {
  ...
  const tachesTerminees = changements.reduce(
    (s, c) => s + c.taches.filter(t => t.statut.code_statut === 'TERMINEE').length, 0
  );
  const totalJournaux = changements.reduce(
    (s, c) => s + c.taches.reduce((ss, t) => ss + t.journaux.length, 0), 0
  );
  const totalTests = changements.reduce((s, c) => s + c.tests.length, 0);
  const testsReussis = changements.reduce(
    (s, c) => s + c.tests.filter(t => t.resultat === 'REUSSI').length, 0
  );
  const hasPir = changements.some(c => c.pir !== null);
  const hasRollback = changements.some(c => c.planRollback !== null);
  const nbVotes = changements.length > 0 ? 0 : 0; // calculé à partir rfcReunions
