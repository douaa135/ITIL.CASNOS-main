...
/**
 * Crée une réunion CAB et envoie les convocations par email
 * à tous les membres permanents du CAB.
 */
async function createReunion(id_cab, data) {
  ...
  // ── Email de convocation à tous les membres du CAB ───────────
  // Non bloquant — un échec SMTP ne fait pas échouer la création
  _envoyerConvocationsCab(id_cab, reunion).catch(
    err => console.error('[CAB] convocation email error :', err.message)
  );
...
/**
 * Récupère les membres, construit la liste des RFCs à l'ordre
 * du jour et envoie les emails de convocation.
 * @private
 */
async function _envoyerConvocationsCab(id_cab, reunion) {
  const cab = await prisma.cab.findUnique({
    where:  { id_cab },
    select: { nom_cab: true },
  });

  const membres = await prisma.membreCab.findMany({
    where:  { id_cab },
    select: { utilisateur: { select: { email_user: true, actif: true } } },
  });

  const emails = membres
    .map(m => m.utilisateur)
    .filter(u => u.actif && u.email_user)
    .map(u => u.email_user);

  if (!emails.length) return;

  // Récupérer les RFCs déjà inscrites à cet ordre du jour (peut être vide à la création)
  const rfcs = (reunion.rfcReunions ?? []).map(r => r.rfc);

  await emailSvc.sendConvocationCabEmail(emails, {
    nom_cab:      cab?.nom_cab ?? 'CAB',
    date_reunion: reunion.date_reunion,
    ordre_jour:   reunion.ordre_jour,
    rfcs,
  });

  console.log(`[CAB] Convocations envoyées à ${emails.length} membre(s) pour la réunion ${reunion.code_metier}`);
}
...
async function castVote(id_reunion, id_rfc, data) {
  ...
  await auditSvc.logAction({
    ...
    id_user,
    ancienne_val: null,
    nouvelle_val: { id_reunion, id_rfc, valeur_vote, rfc_code: vote.rfc?.code_rfc ?? null },
  });
...
/**
 * Enregistre une décision finale, émet en WebSocket,
 * et notifie le demandeur de la RFC par email.
 */
async function createDecision(id_reunion, id_rfc, data) {
  ...
  await auditSvc.logAction({
    ...
    id_user:      null,
    ancienne_val: null,
    nouvelle_val: { id_reunion, id_rfc, decision, motif, rfc_code: dec.rfc?.code_rfc ?? null },
  });
  ...
  // ── Email de résultat → demandeur de la RFC + Change Managers ─
  _envoyerEmailDecision(id_rfc, dec).catch(
    err => console.error('[CAB] decision email error :', err.message)
  );
...
/**
 * Envoie l'email de résultat de décision CAB au demandeur
 * de la RFC et aux Change Managers actifs.
 * @private
 */
async function _envoyerEmailDecision(id_rfc, dec) {
  ...
  // Récupérer les Change Managers actifs
  ...
  const emails = [
    rfc.demandeur?.email_user,
    ...changeManagers.map(u => u.email_user),
  ].filter(Boolean);

  const uniqueEmails = [...new Set(emails)];
  if (!uniqueEmails.length) return;

  const LABELS_DECISION = {
    APPROUVER: 'Approuvée ✅',
    REJETER:   'Rejetée ❌',
    REPORTER:  'Reportée 📅',
  };

  const labelDecision = LABELS_DECISION[dec.decision] || dec.decision;
  const demandeurNom  = rfc.demandeur
    ? `${rfc.demandeur.prenom_user} ${rfc.demandeur.nom_user}`
    : '—';

  // Réutiliser le template RFC statut pour notifier la décision
  await emailSvc.sendRfcStatutEmail(uniqueEmails, {
    id_rfc,
    code_rfc:       rfc.code_rfc,
    titre_rfc:      rfc.titre_rfc,
    nouveau_statut: dec.decision === 'APPROUVER' ? 'APPROUVEE' : dec.decision === 'REJETER' ? 'REJETEE' : 'EVALUEE',
    libelle_statut: `Décision CAB : ${labelDecision}${dec.motif ? ` — ${dec.motif.substring(0, 100)}` : ''}`,
    demandeur:      demandeurNom,
  });

  console.log(`[CAB] Email décision "${dec.decision}" envoyé à ${uniqueEmails.length} destinataire(s) pour RFC ${rfc.code_rfc}`);
}
