...
const updateRfc = async (req, res) => {
  try {
    ...
    const isManager = req.user.roles?.includes('ADMIN') 
    || req.user.roles?.includes('CHANGE_MANAGER') 
    || req.user.roles?.includes('SERVICE_DESK') || req.user.roles?.includes('IMPLEMENTEUR');
...
const cancelRfc = async (req, res) => {
  try {
    ...
    const updated = await rfcService.cancelRfc(req.params.id, req.user.id_user);
