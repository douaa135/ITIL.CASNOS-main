 ...
 *
 * Champs acceptés :
 *   - nom_cab   (optionnel) : string non vide
 *   - type_cab  (optionnel) : NORMAL | URGENT
 *   - membres   (optionnel) : tableau d'opérations
 *     [{ action: 'ADD'|'REMOVE'|'UPDATE', id_user: 'uuid', role?: 'PRESIDENT'|'MEMBRE' }]
 *
 * Règles :
 *   - Au moins nom_cab, type_cab OU membres doit être fourni.
 *   - nom_cab doit être une chaîne non vide si fourni.
...
const validateUpdateCab = (req, res, next) => {
  const { nom_cab, type_cab, membres } = req.body;

  // ── 1. Au moins un champ requis ──────────────────────────────
  if (nom_cab === undefined && type_cab === undefined && membres === undefined) {
    return R.badRequest(
      res,
  ...
  // ── 2. Validation nom_cab ───────────────────────────────────
  if (nom_cab !== undefined) {
    if (typeof nom_cab !== 'string' || !nom_cab.trim()) {
      return R.badRequest(
        res,
        '"nom_cab" doit être une chaîne non vide.',
        'INVALID_NOM_CAB'
      );
    }
  }

  // ── 3. Validation type_cab ───────────────────────────────────
