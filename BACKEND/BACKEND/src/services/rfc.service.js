...
const createRfc = async (data, id_user) => {
  ...
  const { titre_rfc, description, justification, date_souhaitee, urgence, impacte_estimee, ci_ids = [] } = data;
  ...
  return prisma.rfc.create({
    data: {
      ...
      id_priorite:     prioriteParDefaut.id_priorite,
      id_type:         typeStandard.id_type,
      id_user,
...
// ─────────────────────────────────────────────────────────────
async function updateRfcStatus(id_rfc, id_statut, id_change_manager, id_env) {
