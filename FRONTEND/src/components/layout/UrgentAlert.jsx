...
async function computeUrgentCount(user) {
  ...
  try {
    ...
    if (isAdmin || isCM || isSD || isCAB || isDem) {
      ...
      let urgentFiltered = [];
      if (isCM && !isAdmin) {
        // Le Change Manager ne compte que les RFC urgentes au statut PRE_APPROUVEE
        urgentFiltered = urgentRfcs.filter(r => r.statut?.code_statut === 'PRE_APPROUVEE');
      } else {
        ...
        urgentFiltered = urgentRfcs.filter(r => targetStatuses.has(r.statut?.code_statut));
      }
