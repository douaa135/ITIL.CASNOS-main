    ...
    <div className="my-tasks-container">
      ...
      <div className="tasks-list-panel">
           ...
           <div style={{ display: 'flex', gap: '0.75rem' }}>
             ...
             {(search || statusFilter || typeFilter || kpiFilter) && (
               <button
                 ...
                 style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }} className="reset-filters-btn-cab"
                 title="Réinitialiser"
               >
        ...
        <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          {loading ? (
            <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>Initialisation...</div>
          ) : filteredTasks.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
              <FiCheckCircle size={40} style={{ display: 'block', margin: '0 auto 1rem', opacity: 0.3 }} />
              ...
              <table style={{ width: '100%', minWidth: '900px', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#64748b', textAlign: 'left', whiteSpace: 'nowrap' }}>Changement</th>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#64748b', textAlign: 'left', whiteSpace: 'nowrap' }}>Type</th>
                    ...
                    <th style={{ padding: '12px 16px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#64748b', textAlign: 'left', whiteSpace: 'nowrap' }}>Durée</th>
                ...
                <tbody>
                  {filteredTasks.map(task => {
                    ...
                    const statusLabel = task.statut?.libelle || task.statut || 'N/A';
                    ...
                    else if (['EN_ATTENTE'].includes(statusCode)) { statusBg = 'rgba(59,130,246,0.1)'; statusFg = '#3b82f6'; }

                    const type = (task.changement?.rfc?.typeRfc?.type || 'STANDARD').toUpperCase();
                    const typeColors = { URGENT: { bg: '#fef2f2', color: '#ef4444', border: '#fee2e2' }, NORMAL: { bg: '#eff6ff', color: '#3b82f6', border: '#dbeafe' }, STANDARD: { bg: '#f8fafc', color: '#64748b', border: '#e2e8f0' } };
                    const tc = typeColors[type] || typeColors.STANDARD;
                    ...
                    return (
                      <tr
                        ...
                        style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer', background: selectedTask?.id_tache === task.id_tache ? '#eff6ff' : '#ffffff', transition: 'background 0.15s' }}
                        onMouseEnter={e => { if (selectedTask?.id_tache !== task.id_tache) e.currentTarget.style.background = '#f8fafc'; }}
                        onMouseLeave={e => { e.currentTarget.style.background = selectedTask?.id_tache === task.id_tache ? '#eff6ff' : '#ffffff'; }}
                      >
                        {/* Titre & Code — sticky */}
                        <td style={{ position: 'sticky', left: 0, zIndex: 2, background: 'inherit', padding: '14px 16px', borderRight: '1px solid #f1f5f9', whiteSpace: 'nowrap' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 700, color: '#0f172a', fontSize: '0.82rem', maxWidth: '220px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '3px' }} title={task.titre_tache}>{task.titre_tache}</div>
                        ...
                        {/* RFC */}
                        <td style={{ padding: '14px 16px', fontSize: '0.875rem', color: '#334155', verticalAlign: 'middle' }}>
                          <span style={{ fontWeight: 600 }}>{task.changement?.rfc?.code_rfc || '-'}</span>
                        </td>

                        {/* Type */}
                        <td style={{ padding: '14px 16px', verticalAlign: 'middle' }}>
                          <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700, background: tc.bg, color: tc.color, border: `1px solid ${tc.border}`, whiteSpace: 'nowrap' }}>{type}</span>
                        </td>
                        ...
                        {/* Durée */}
                        <td style={{ padding: '14px 16px', fontSize: '0.8rem', color: '#64748b', verticalAlign: 'middle' }}>
                          {task.duree ? `${task.duree}h` : '-'}
                        </td>
      ...
      {pirForm.show && (
        <div className="modal-overlay" onClick={() => setPirForm({ show: false })}>
          <div className="modal-content" style={{ borderRadius: '20px', overflow: 'hidden', maxWidth: '640px' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: pirForm.isEchec ? '#dc2626' : '#16a34a', borderBottom: `1px solid ${pirForm.isEchec ? '#b91c1c' : '#15803d'}`, padding: '1.5rem 2rem' }}>
              <div className="rfc-style-icon-wrapper" style={{ background: 'rgba(255,255,255,0.1)', color: '#ffffff', borderColor: 'rgba(255,255,255,0.2)' }}>
            ...
            <div className="modal-body" style={{ background: '#f0f9ff' }}>
              ...
              <div className="modal-actions">
                <button 
                  className={pirForm.isEchec ? 'btn-probleme' : 'btn-cloturer'} 
