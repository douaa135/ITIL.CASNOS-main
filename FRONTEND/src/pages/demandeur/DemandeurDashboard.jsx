...
import { 
  ...
  FiZap, FiInfo, FiUser, FiSend, FiRefreshCw
} from 'react-icons/fi';
...
const DemandeurDashboard = () => {
  ...
  const [stats, setStats] = useState({ total: 0, soumises: 0, inProgress: 0, finalized: 0 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { rfcs: list, stats: s } = await rfcService.getDemandeurStats(user.id_user);
        setRfcs(list || []);
        setStats(s);
      } catch (err) {
  ...
  if (loading) return (
    <div className="impl-loading">
      <FiRefreshCw className="spinning" /> Chargement du cockpit demandeur...
    </div>
  );
