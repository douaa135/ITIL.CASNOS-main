...
const RfcCreate = ({ isModal = false, onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    if (!isEdit && !initialRfc) {
      const draft = localStorage.getItem('rfc_create_draft');
      if (draft) {
        try {
  ...
  useEffect(() => {
    if (!isEdit && !submitting) {
      localStorage.setItem('rfc_create_draft', JSON.stringify({ form, step }));
    }
  }, [form, step, isEdit, submitting]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        // Si c'est un Change Manager ou Admin qui crée, on passe direct en PRE_APPROUVEE
        code_statut:    (userRole === 'CHANGE_MANAGER' || userRole === 'ADMIN') ? 'PRE_APPROUVEE' : undefined
      };
      ...
      if (isEdit) {
        ...
        localStorage.removeItem('rfc_create_draft'); // Clear draft on success
      }
