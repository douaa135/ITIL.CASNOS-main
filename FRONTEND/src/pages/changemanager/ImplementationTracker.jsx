...
const ImplementationTracker = () => {
        ...
        const initData = async () => {
            ...
            try {
                ...
                implementersList.forEach(imp => {
                    const impTasks = Object.values(tasksMap).flat().filter(
                        t => t.id_user === imp.id_user || t.implementeur?.id_user === imp.id_user
                    );
                    ...
                    statsMap[imp.id_user] = {
                        ...
                        urgent: impTasks.filter(t => {
                            ...
                            const isUrgentTask = ['HAUTE', 'CRITIQUE', 'P0', 'P1'].includes(taskPrio) || 
                                                ['HAUTE', 'CRITIQUE', 'P0', 'P1'].includes(chgPrio);
                            
                            return isPending && (isUrgentRfc || isUrgentTask);
                        }).length,
                        completed: impTasks.filter(t => ['TERMINEE', 'CLOTUREE', 'SUCCES'].includes(t.statut?.code_statut)).length,
                    };
                });
    ...
    const fetchImplementerTasks = (imp) => {
        ...
        const allTasksForUser = Object.values(tasksData).flat().filter(
            t => t.id_user === imp.id_user || t.implementeur?.id_user === imp.id_user
        );
    ...
    const filteredChangements = useMemo(() => {
        return changements.filter((change) => {
            ...
            if (kpiFilter === 'ROLLBACK') {
                return ['ROLLBACK', 'EN_ECHEC', 'ANNULEE'].includes(change.statut?.code_statut) && matchesSearch && matchesType && matchesStatus;
            }
                    ...
                    <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '1rem', marginBottom: '0.5rem' }}>
                        <div className={`stat-card blue clickable ${kpiFilter === 'EN_COURS' ? 'active' : ''}`} 
                            ...
                            <div className="stat-info">
                                <div className="stat-value" style={{ color: '#2563eb' }}>{changements.filter(c => c.statut?.code_statut === 'EN_COURS').length}</div>
                        ...
                        <div className={`stat-card amber clickable ${kpiFilter === 'EN_ATTENTE' ? 'active' : ''}`}
                            ...
                            <div className="stat-info">
                                <div className="stat-value" style={{ color: '#d97706' }}>{changements.filter(c => c.statut?.code_statut === 'EN_ATTENTE').length}</div>
                            ...
                            <div className="stat-info">
                                <div className="stat-value" style={{ color: '#dc2626' }}>
                                    {changements.filter(change => {
                                        ...
                                        return (isUrgent || hasUrgentTask) && !['CLOTUREE', 'CLOTURE', 'IMPLEMENTE', 'TERMINEE', 'REUSSI', 'ECHEC', 'ANNULEE', 'REJETEE'].includes(change.statut?.code_statut);
                                    }).length}
                                </div>
                        ...
                        <div className={`stat-card green clickable ${kpiFilter === 'TERMINEE' ? 'active' : ''}`}
                            ...
                            <div className="stat-info">
                                <div className="stat-value" style={{ color: '#059669' }}>{changements.filter(c => ['TERMINEE', 'CLOTUREE', 'IMPLEMENTE', 'REUSSI'].includes(c.statut?.code_statut)).length}</div>
                        ...
                        <div className={`stat-card red clickable ${kpiFilter === 'ROLLBACK' ? 'active' : ''}`}
                            ...
                            <div className="stat-info">
                                <div className="stat-value" style={{ color: '#dc2626' }}>{changements.filter(c => ['ROLLBACK', 'EN_ECHEC', 'ANNULEE'].includes(c.statut?.code_statut)).length}</div>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem' }}>
                            ...
                            {kpiFilter === 'URGENT' && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', background: '#fef2f2', color: '#dc2626', borderRadius: '12px', border: '1px solid #fee2e2', fontWeight: 700, fontSize: '0.9rem' }}>
                                    ...
                                    <button onClick={() => setKpiFilter('')} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', display: 'flex', padding: 0 }}><FiX /></button>
                                </div>
                            )}
                        ...
                        <div className="modal-backdrop-center" onClick={() => setShowTasksModal(false)}>
                            <div className="tasks-modal-content" style={{ background: '#f0f9ff', border: '1px solid #bae6fd' }} onClick={(e) => e.stopPropagation()}>
                                <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
                                    ...
                                    <button type="button" className="close-btn-rfc-style" onClick={() => setShowTasksModal(false)} style={{ color: '#ffffff' }}>
                                        <FiX size={24} />
                                    </button>
                                </div>
                ...
                <div className="modal-overlay">
                    <div className="modal-box-cab glass-card-cab tm-modal-large" style={{ maxWidth: '700px', background: '#f0f9ff', border: '1px solid #bae6fd' }}>
                        <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
                            ...
                            <div className="rfc-style-header-text">
                                <h2 style={{ color: '#ffffff' }}>{resultModal.title}</h2>
                                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Tâche: {resultModal.task?.titre_tache} (#{resultModal.task?.code_tache})</div>
                            </div>
                        ...
                        <div className="modal-body-rfc-style" id="report-content-to-print">
                            <div className="tm-detail-grid" style={{ marginBottom: '1.5rem' }}>
                                <div className="tm-detail-box">
                                    ...
                                    <div style={{ fontWeight: '600' }}>{resultModal.task?.implementeur?.prenom_user} {resultModal.task?.implementeur?.nom_user}</div>
                                </div>
                                <div className="tm-detail-box">
                                    <div style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748b', marginBottom: '0.25rem' }}>Statut Final</div>
                                    <div style={{ fontWeight: '600' }}>{resultModal.task?.statut?.libelle || resultModal.task?.statut}</div>
                                </div>
                            </div>
