...
const RfcManagement = () => {
  ...
  const handleApproveRfc = async (rfc, pendingId) => {
    ...
    if (!envToUse) {
      showToast("Veuillez sélectionner un environnement dans les paramètres ITIL avant d'approuver.", 'warning');
      return;
    }
  ...
  // ── Filtrage — RFC à évaluer et Évaluées ──
  ...
  const filtered = rfcs.filter(r => {
    ...
    if (kpiFilter === 'URGENT') {
      ...
      matchStatus = isUrgent && (r.statut?.code_statut === 'SOUMIS' || r.statut?.code_statut === 'PRE_APPROUVEE' || r.statut?.code_statut === 'EVALUEE');
  ...
  const kpiUrgent = rfcs.filter(r => {
    ...
    const isUrgent = typeStr.includes('URGENT') || 
                     prioStr.includes('URGENT') || prioStr.includes('HAUTE') || prioStr.includes('CRITIQUE') || prioStr.includes('P0') || prioStr.includes('P1') ||
                     r.urgence === true || r.urgence === 1 || String(r.urgence) === 'true';
    return isUrgent && (r.statut?.code_statut === 'SOUMIS' || r.statut?.code_statut === 'PRE_APPROUVEE' || r.statut?.code_statut === 'EVALUEE');
  }).length;
                ...
                >
                  ...
                  <td style={{ position: 'sticky', right: 0, zIndex: 2, background: 'inherit', padding: '14px 16px', borderLeft: '1px solid #f1f5f9' }} onClick={e => e.stopPropagation()}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '6px' }}>
                      ...
                      {(() => {
                        const canDelete = ['SOUMIS', 'PRE_APPROUVEE'].includes(rfc.statut?.code_statut);
                        return (
                          <button
                            onClick={e => {
                              e.stopPropagation();
