...
const RfcManagement = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // On enrichit les RFC avec l'id_env correct
      ...
      // Injection de la RFC demandée par l'utilisateur
      const sampleRfc = {
        id_rfc: 'SEED-012',
        code_rfc: 'RFC-SEED-012',
        titre_rfc: 'Mise à jour LDAP — OpenLDAP 2.6 vers 2.7',
        description: 'Mise à jour critique du service LDAP pour supporter les nouvelles fonctionnalités de sécurité et optimiser les performances de synchronisation.',
        justification: 'Amélioration de la sécurité globale et support des nouveaux protocoles d\'authentification.',
        statut: { code_statut: 'PRE_APPROUVEE', libelle: 'À évaluer' },
        typeRfc: { type: 'MINEUR', code_type: 'MINEUR' },
        priorite: { libelle: 'HAUTE', code_priorite: 'HAUTE' },
        date_souhaitee: '2026-05-20T09:00:00Z',
        demandeur: { prenom_user: 'Admin', nom_user: 'Système', direction: { nom_direction: 'Direction Technique' } }
      };
      
      if (!enrichedRfcs.some(r => r.code_rfc === 'RFC-SEED-012')) {
        enrichedRfcs.unshift(sampleRfc);
      }
  ...
  const fetchComments = async (id) => {
    try {
      ...
      if (r) {
        setRisk({
          impact: r.impacte || r.impact || 1,
          probabilite: r.probabilite || 1,
          score: r.score_risque || r.score || 1,
          notes: r.description || r.notes || ''
        });
      } else {
        setRisk({ impact: 1, probabilite: 1, score: 1, notes: '' });
      }
      setHistory(h);
  ...
  const handleUpdateDetail = async () => {
    try {
      ...
      // Alignement avec les clés attendues par le backend (support des deux formats : impacte/probabilite et _impacte/_probabilite)
      const sanitizedRisk = {
        impacte: risk.impact,
  ...
  const handleDecision = async (newStatusCode) => {
    try {
      ...
      // Enregistrement automatique du risque
      if (risk && risk.impact && risk.probabilite) {
        try {
          await rfcService.upsertEvaluationRisque(selectedRfc.id_rfc, {
            impacte: risk.impact,
            probabilite: risk.probabilite,
            score_risque: risk.score || (risk.impact * risk.probabilite),
            _impacte: risk.impact,
            _probabilite: risk.probabilite,
            _score: risk.score || (risk.impact * risk.probabilite),
            description: risk.notes || ""
          });
        } catch (_) {}
      }
  ...
  const handleApproveRfc = async (rfc, pendingId) => {
    // Enregistrement automatique du risque
    if (risk && risk.impact && risk.probabilite) {
      try {
    ...
    // On utilise l'environnement sélectionné dans le modal de traitement (detailForm ou selectedEnv)
    const envToUse = selectedEnv || rfc.id_env || rfc.environnement?.id_env;
    
    if (!envToUse) {
      showToast("Veuillez sélectionner un environnement dans les paramètres ITIL avant d'approuver.", 'error');
      return;
    }

    setApprovalLoading(true);
    try {
      const res = await rfcService.updateRfcStatus(rfc.id_rfc, pendingId, {
        id_env: envToUse,
        id_change_manager: user.id_user,
        date_debut: rfc.date_souhaitee || new Date().toISOString()
      });

      // Si le backend n'a pas créé le changement automatiquement, on le fait manuellement
      if (!res?.changement && !res?.data?.changement) {
        try {
          ...
          console.warn("Manual change creation failed (likely already exists):", manualErr);
        }
      }
  ...
  const sortedStatuses = useMemo(() => {
    const allowed = ['PRE_APPROUVEE', 'EVALUEE'];
    return statuses.filter(s => allowed.includes(s.code_statut));
  }, [statuses]);

  // ── Filtrage — RFC à évaluer et Pré-approuvées ──
  const ALLOWED_STATUSES = ['PRE_APPROUVEE', 'EVALUEE'];
  ...
  const filtered = rfcs.filter(r => {
    ...
    if (kpiFilter === 'URGENT') {
      ...
      matchStatus = isUrgent && (r.statut?.code_statut === 'PRE_APPROUVEE' || r.statut?.code_statut === 'EVALUEE');
    ...
    const matchType = !filterType || 
      (filterType.toUpperCase() === 'URGENT'
        ? ( (r.typeRfc?.type || '').toUpperCase().includes('URGENT') || r.urgence === true || r.urgence === 1 || String(r.urgence) === 'true' )
  ...
  const kpiUrgent = rfcs.filter(r => {
    const typeStr = (r.typeRfc?.type || r.type || r.typeRfc?.code_type || '').toUpperCase();
    const prioStr = String(r.priorite?.libelle || r.priorite?.code_priorite || r.id_priorite || '').toUpperCase();
    const isUrgent = typeStr.includes('URGENT') || 
                     prioStr.includes('URGENT') || prioStr.includes('HAUTE') || prioStr.includes('CRITIQUE') || prioStr.includes('P0') || prioStr.includes('P1') ||
                     r.urgence === true || r.urgence === 1 || String(r.urgence) === 'true';
    return isUrgent && (r.statut?.code_statut === 'PRE_APPROUVEE' || r.statut?.code_statut === 'EVALUEE');
  }).length;
  
  const kpiPreApprouvee = rfcs.filter(r => r.statut?.code_statut === 'PRE_APPROUVEE').length;
  const kpiEvaluee      = rfcs.filter(r => r.statut?.code_statut === 'EVALUEE').length;
  const kpiLate         = rfcs.filter(r => isLate(r) && (r.statut?.code_statut === 'PRE_APPROUVEE' || r.statut?.code_statut === 'EVALUEE')).length;
  ...
  return (
    <div className="rfc-mgr-page">
      ...
      <div className="premium-header-card">
        <div className="premium-header-left">
          ...
          <div className="premium-header-text">
            <h1>Gestion RFC</h1>
            <p>Analysez l'impact, évaluez les risques et préparez les décisions pour les demandes de changement</p>
          </div>
        </div>
      ...
      {/* ── KPI CARDS ──────────────────────────────────────── */}
      <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '0.5rem' }}>
        <div className={`stat-card red clickable ${kpiFilter === 'URGENT' ? 'active' : ''}`} 
          ...
          <div className="stat-icon-wrapper" style={{ background: '#fef2f2', color: '#dc2626' }}><FiZap size={22} /></div>
        ...
        <div className={`stat-card amber clickable ${kpiFilter === 'PRE_APPROUVEE' ? 'active' : ''}`}
             onClick={() => setKpiFilter(kpiFilter === 'PRE_APPROUVEE' ? '' : 'PRE_APPROUVEE')}
             style={{ borderLeft: '4px solid #f59e0b', cursor: 'pointer' }}>
          <div className="stat-icon-wrapper" style={{ background: '#fffbeb', color: '#d97706' }}><FiClock size={22} /></div>
          <div className="stat-info">
            <div className="stat-value" style={{ color: '#d97706' }}>{kpiPreApprouvee}</div>
            <div className="stat-label">À évaluer</div>
          </div>
        </div>

        <div className={`stat-card blue clickable ${kpiFilter === 'EVALUEE' ? 'active' : ''}`}
             onClick={() => setKpiFilter(kpiFilter === 'EVALUEE' ? '' : 'EVALUEE')}
             style={{ borderLeft: '4px solid #3b82f6', cursor: 'pointer' }}>
          <div className="stat-icon-wrapper" style={{ background: '#eff6ff', color: '#2563eb' }}><FiCheckCircle size={22} /></div>
          <div className="stat-info">
            <div className="stat-value" style={{ color: '#2563eb' }}>{kpiEvaluee}</div>
            <div className="stat-label">Pré-aprouver</div>
          </div>
        </div>

        <div className={`stat-card orange clickable ${kpiFilter === 'LATE' ? 'active' : ''}`}
             onClick={() => setKpiFilter(kpiFilter === 'LATE' ? '' : 'LATE')}
             style={{ borderLeft: '4px solid #f97316', cursor: 'pointer' }}>
          <div className="stat-icon-wrapper" style={{ background: '#fff7ed', color: '#ea580c' }}><FiAlertCircle size={22} /></div>
          <div className="stat-info">
            <div className="stat-value" style={{ color: '#ea580c' }}>{kpiLate}</div>
      ...
      <PremiumToolbar 
        searchProps={{
          ...
          placeholder: "Rechercher une RFC (code, titre, demandeur)..."
        }}
        filters={[
          {
            value: filterStatus,
            ...
            <tbody>
              {loading ? (
                ...
                >
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700, background: '#f5f3ff', color: '#7c3aed', border: '1px solid #ddd6fe' }}>
                        {rfc.demandeur?.direction?.nom_direction || usersMap[rfc.id_user] || usersMap[rfc.demandeur?.id_user] || '—'}
                      </span>
                    </div>
                  </td>
                  ...
                  <td style={tdStyle}>
                    {(() => {
                        const tId = rfc.id_type || rfc.typeRfc?.id_type;
                        const t = rfcTypes.find(tp => String(tp.id_type) === String(tId));
                        const label = t?.type || 'STANDARD';
                        return (
                          <span className={`type-badge type-${label.toLowerCase()}`}>
                            {label}
                          </span>
                        );
                      })()}
                  ...
                  <td style={tdStyle}>
                    {(() => {
                        ...
                        const colors = {
                          'FAIBLE':   { bg: '#f0fdf4', color: '#16a34a', border: '#dcfce7' }, // Vert clair
                          'BASSE':    { bg: '#dcfce7', color: '#15803d', border: '#bbf7d0' }, // Vert foncé
                          'MOYENNE':  { bg: '#fefce8', color: '#a16207', border: '#fef9c3' }, // Jaune
                          'NORMAL':   { bg: '#fefce8', color: '#a16207', border: '#fef9c3' }, // Jaune
                          'HAUTE':    { bg: '#fff7ed', color: '#ea580c', border: '#ffedd5' }, // Orange
                          'URGENT':   { bg: '#fef2f2', color: '#dc2626', border: '#fee2e2' }, // Rouge
                          'CRITIQUE': { bg: '#fef2f2', color: '#dc2626', border: '#fee2e2' }, // Rouge
                          'P0':       { bg: '#fef2f2', color: '#dc2626', border: '#fee2e2' }, // Rouge
                        };
          ...
          <div className="modal-box modal-box-pir" style={{ background: '#f0f9ff', border: '1px solid #003366' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              ...
              <div className="rfc-style-header-text">
                ...
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>RFC #{selectedRfc?.code_rfc}</div>
              </div>
          ...
          <div className="modal-box" style={{ maxWidth: '900px', width: '95%', display: 'flex', flexDirection: 'column', maxHeight: '95vh', padding: 0, background: '#f0f9ff', border: '1px solid #003366' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              ...
              <div className="rfc-style-header-text">
                ...
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Initiation d'une nouvelle demande</div>
              </div>
              <button onClick={closeModals} className="close-btn-rfc-style" style={{ color: '#ffffff', background: 'none', border: 'none', cursor: 'pointer' }}>
                <FiX size={24} />
              </button>
            </div>
