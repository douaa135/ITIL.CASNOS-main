...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            setLoading(true);
            try {
                const { rfcs, stats: calculatedStats } = await rfcService.getChangeManagerStats();
    ...
    return (
        <div className="manager-dashboard">
            ...
            <div className="stats-grid">
                <StatCard title="RFC Actives" value={stats.total} icon={<FiFileText />} color="blue" onClick={() => navigate('/manager/rfcs')} />
                <StatCard title="En attente" value={stats.pending} icon={<FiClock />} color="amber" trend={{ value: 'Evaluation', type: 'warning' }} onClick={() => navigate('/manager/rfcs', { state: { filterStatus: 'PRE_APPROUVEE' } })} />
                <StatCard title="RFC Urgentes" value={stats.urgent} icon={<FiAlertCircle />} color="red" trend={{ value: 'Priorité Haute', type: 'danger' }} onClick={() => navigate('/manager/rfcs', { state: { filterType: 'URGENT' } })} />
                <StatCard title="Approuvées" value={stats.approved} icon={<FiCheckCircle />} color="green" trend={{ value: 'Prêt FSC', type: 'success' }} onClick={() => navigate('/manager/changements')} />
            </div>
