...
// ── Rendu de l'objet avec le code en bleu ──────────────────
...
const BroadcastCenter = () => {
  ...
  const handleRowClick = async (notif) => {
    ...
    if (activeTab === 'reception' && !notif.lue) {
      try {
        const id = notif.id_notif ?? notif.id;
        await api.patch(`/notifications/${id}/read`);
        setReceptionNotifs(prev =>
          prev.map(n => (n.id_notif === id || n.id === id) ? { ...n, lue: true } : n)
        );
      } catch (e) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const validUsers = (Array.isArray(list) ? list : []).filter(u => u.email_user !== 'systeme@itil.internal');
      setUsers(validUsers);
  ...
  return (
    <div className="broadcaster-page">
      ...
      <PremiumToolbar 
        ...
        extraContent={
          <div className="premium-toolbar-filter" style={{ flex: 0.6, minWidth: '160px' }}>
      ...
      {/* ── TABLE ── */}
      <div className="broadcast-table-card table-scroll-container">
        <div style={{ overflowX: 'auto', scrollBehavior: 'smooth' }}>
          {activeTab === 'reception' ? (
            <table className="broadcast-table" style={{ minWidth: '750px' }}>
              <thead>
                <tr>
                  <th>Objet</th>
                  <th>Message</th>
              ...
              <tbody>
                {receptionLoading ? (
                  ...
                  <tr key={n.id_notif ?? n.id}
                    className={n.lue ? 'read-row' : 'unread-row'}
                    onClick={() => handleRowClick(n)}
                  >
                    <td>
                      <div style={{ fontWeight: n.lue ? 600 : 800, display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {!n.lue && <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#7c3aed', flexShrink: 0, display: 'inline-block' }} />}
                        {renderObjetWithBlueCode(n.objet)}
                      </div>
                    </td>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      <span style={{ padding: '6px 16px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 700, background: n.lue ? '#f0fdf4' : '#faf5ff', color: n.lue ? '#16a34a' : '#7c3aed', border: `1.5px solid ${n.lue ? '#bbf7d0' : '#ddd6fe'}`, display: 'inline-block', minWidth: '70px', textAlign: 'center' }}>
                        {n.lue ? 'Lu' : 'Non lu'}
                      </span>
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <button
                        onClick={(e) => { e.stopPropagation(); setConfirmDelete({ open: true, id: n.id_notif ?? n.id }); }}
            ...
            <table className="broadcast-table" style={{ minWidth: '700px' }}>
              <thead>
                <tr>
                  <th>Objet</th>
                  <th>Message</th>
              ...
              <tbody>
                {sentLoading ? (
                  <tr><td colSpan={4} style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>Chargement...</td></tr>
                ) : paginated.length === 0 ? (
                  <tr>
                  ...
                  return (
                    ...
                    >
                      <td>
                        <div style={{ fontWeight: 700 }}>{renderObjetWithBlueCode(n.objet)}</div>
                      </td>
                      <td style={{ maxWidth: 280 }}>
                        <div style={{ color: '#64748b', fontSize: '0.88rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 260 }}>
                          {n.message}
                        </div>
                      </td>
                      ...
                      <td>
                        {recipients.length === 0 ? (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <span style={{ fontWeight: 600, color: '#1e293b', fontSize: '0.93rem' }}>{recipientLabel}</span>
                          </div>
                        ) : (
                          <button
                            onClick={(e) => { e.stopPropagation(); setRecipientsModal({ open: true, list: recipients, objet: n.objet ?? '' }); }}
                            style={{
                              padding: '4px 12px', borderRadius: '20px', background: '#eff6ff',
                              color: '#1d4ed8', border: '1px solid #bfdbfe', fontSize: '0.82rem',
        ...
        >
          ...
          >
            ...
            <div className="premium-modal-blue-header">
              ...
              <button onClick={() => setSelectedNotif(null)} className="close-btn-rfc-style" style={{ color: '#fff' }}>
                <FiX size={20} />
              </button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '2rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                {/* Info block (Expéditeur / Destinataires) */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', background: '#f8fafc', padding: '1.25rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                  <div>
                    ...
                    {selectedNotif.expediteur ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <Avatar prenom={selectedNotif.expediteur.prenom_user} nom={selectedNotif.expediteur.nom_user} />
                        <div>
                          ...
                          <div style={{ fontSize: '0.75rem', color: '#64748b' }}>
                            {ROLE_LABELS[selectedNotif.expediteur.roles?.[0]] ?? selectedNotif.expediteur.roles?.[0] ?? 'Utilisateur'}
                          </div>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div>
                    <h5 style={{ margin: '0 0 0.5rem 0', fontSize: '0.7rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Statut / Diffusion</h5>
                    {activeTab === 'reception' ? (
                      <span style={{ padding: '6px 16px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 700, background: selectedNotif.lue ? '#f0fdf4' : '#faf5ff', color: selectedNotif.lue ? '#16a34a' : '#7c3aed', border: `1.5px solid ${selectedNotif.lue ? '#bbf7d0' : '#ddd6fe'}`, display: 'inline-block' }}>
                        {selectedNotif.lue ? 'Lu' : 'Non lu'}
                      </span>
                    ) : (
                      <div>
                        <div style={{ fontWeight: 700, color: '#1d4ed8', fontSize: '0.88rem' }}>
                          {selectedNotif.destinataires?.length ?? 0} destinataire(s)
                        </div>
                        {selectedNotif.destinataires && selectedNotif.destinataires.length > 0 && (
                          <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '2px' }}>
                            {selectedNotif.destinataires.slice(0, 2).map(d => `${d.destinataire?.prenom_user ?? ''} ${d.destinataire?.nom_user ?? ''}`).join(', ')}
                            {selectedNotif.destinataires.length > 2 ? '...' : ''}
                          </div>
                        )}
                      </div>
                    )}
                ...
                {/* Destinataires list (only if it's sent and has multiple recipients) */}
                {activeTab === 'envoi' && selectedNotif.destinataires && selectedNotif.destinataires.length > 0 && (
                  <div>
                    <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Liste complète des destinataires</h4>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', maxHeight: '150px', overflowY: 'auto', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '10px', background: '#f8fafc' }}>
                      {selectedNotif.destinataires.map((d, i) => {
                        const u = d.destinataire ?? d;
                        return (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'white', padding: '4px 8px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '0.75rem' }}>
                            <Avatar prenom={u.prenom_user} nom={u.nom_user} />
                            <span style={{ fontWeight: 600, color: '#1e293b' }}>{u.prenom_user} {u.nom_user}</span>
                          </div>
                        );
                      })}
                    </div>
