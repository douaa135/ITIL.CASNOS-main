...
const CreateCiModal = ({ onClose, onCreated, environnements }) => {
  ...
  return (
    <div className="modal-backdrop-cab" onClick={onClose}>
      <div className="modal-box-cab glass-card-cab ci-modal-medium" style={{ background: '#f0f9ff', border: '1px solid #003366' }} onClick={e => e.stopPropagation()}>
        <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855' }}>
          <div className="rfc-style-icon-wrapper" style={{ background: 'rgba(255, 255, 255, 0.1)', color: '#ffffff', borderColor: 'rgba(255, 255, 255, 0.2)' }}><FiDatabase /></div>
          <div className="rfc-style-header-text">
            <h2 style={{ color: '#ffffff' }}>Nouveau Configuration Item</h2>
            <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>Enregistrez un élément dans le référentiel CMDB</div>
          </div>
...
const EditCiModal = ({ ci, onClose, onUpdated, environnements }) => {
  ...
  return (
    <div className="modal-backdrop-cab" onClick={onClose}>
      <div className="modal-box-cab glass-card-cab ci-modal-medium" style={{ background: '#f0f9ff', border: '1px solid #003366' }} onClick={e => e.stopPropagation()}>
        <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855' }}>
          <div className="rfc-style-icon-wrapper ci-edit-icon" style={{ background: 'rgba(255, 255, 255, 0.1)', color: '#ffffff', borderColor: 'rgba(255, 255, 255, 0.2)' }}><FiEdit2 /></div>
          <div className="rfc-style-header-text">
            <h2 style={{ color: '#ffffff' }}>Modifier le CI</h2>
            <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{ci.nom_ci} — #{ci.code_metier}</div>
          </div>
          <button className="close-btn-rfc-style" onClick={onClose} style={{ color: '#ffffff' }}>
            <FiX size={24} />
          </button>
...
const DetailCiModal = ({ ci, onClose, onEdit, onDelete }) => {
  ...
  return (
    <div className="modal-backdrop-cab" onClick={onClose}>
      <div className="modal-box-cab glass-card-cab ci-modal-large" style={{ background: '#f0f9ff', border: '1px solid #003366' }} onClick={e => e.stopPropagation()}>
        <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855' }}>
          <div className="rfc-style-icon-wrapper ci-detail-icon" style={{ background: 'rgba(255, 255, 255, 0.1)', color: '#ffffff', borderColor: 'rgba(255, 255, 255, 0.2)' }}>
            {getCiIcon(ci.type_ci)}
          </div>
          <div className="rfc-style-header-text">
            <h2 style={{ color: '#ffffff' }}>Fiche technique de l'environnement</h2>
            <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>Détails du CI — #{ci.code_metier} — {ci.nom_ci}</div>
          </div>
          <button className="close-btn-rfc-style" onClick={onClose} style={{ color: '#ffffff' }}>
            <FiX size={24} />
          </button>
        ...
        <div className="modal-body-rfc-style">
          ...
          <div className="premium-detail-box premium-detail-box-spacing-lg" style={{ background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: '12px', padding: '1.25rem' }}>
            <div className="premium-detail-label" style={{ color: '#0369a1', marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <FiLayers size={14} /> Environnements & Architecture
            </div>
            <div className="ci-env-list ci-env-list-detail" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {ci.ciEnvs?.length > 0 ? ci.ciEnvs.map(e => (
                <span key={e.environnement?.id_env || e.id_env} className="ci-env-pill ci-env-pill-detail" style={{ background: '#ffffff', color: '#0369a1', border: '1.5px solid #bae6fd', padding: '0.4rem 0.8rem', borderRadius: '8px', fontWeight: '700', fontSize: '0.8rem' }}>
                  {e.environnement?.nom_env || 'Inconnu'}
                </span>
              )) : <span className="ci-muted-italic">Aucun environnement spécifié dans la fiche technique.</span>}
            </div>
          </div>
      ...
      <PremiumToolbar 
        ...
        filters={[
          ...
          },
          {
            ...
            options: environnements.map(env => ({ value: env.id_env, label: env.nom_env }))
          }
        ]}
        onReset={() => { setSearch(''); setFilterEnv('ALL'); setFilterType('ALL'); }}
        showReset={!!(search || filterEnv !== 'ALL' || filterType !== 'ALL')}
      />
