...
const CabMeetingsAdmin = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const filteredUsers = (Array.isArray(rawUsers) ? rawUsers : []).filter(u => 
        (u.nom_user + ' ' + u.prenom_user).toLowerCase().indexOf('itil') === -1 &&
        (u.nom_user + ' ' + u.prenom_user).toLowerCase().indexOf('systeme') === -1
      );
      setAllUsers(filteredUsers);
      
      setStatuses(Array.isArray(statutsRes) ? statutsRes : []);
  ...
  const handleFinalDecision = async (rfc, statusCode) => {
    try {
      ...
      // Enregistrer la décision officielle CAB
      ...
      try {
        ...
        console.warn('Décision CAB déjà enregistrée ou erreur:', decisionErr);
      }
      ...
      if (statusCode === 'APPROUVEE') {
        if (processRisk && processRisk.impact && processRisk.probabilite) {
          try {
            await rfcService.upsertEvaluationRisque(rfc.id_rfc, {
              ...
              _impacte: processRisk.impact,
              _probabilite: processRisk.probabilite,
              _score: processRisk.score || (processRisk.impact * processRisk.probabilite),
        ...
        try {
          ...
          console.warn("Change creation failed:", e);
        }
      }

      setRfcs(prev => prev.filter(r => r.id_rfc !== rfc.id_rfc));
      setToast({ msg: `RFC ${statusCode === 'APPROUVEE' ? 'Approuvée' : 'Rejetée'} et traitée !`, type: 'success' });
  ...
  const totalAttendees = cabMembresCount + participants.length;
                      ...
                      <>
                        ...
                        {rfcs.filter(r => {
                          ...
                          <div className="cm-votes-detail">
                            ...
                            {/* Final Decision Buttons for Admin / CAB President */}
...
export default CabMeetingsAdmin;
