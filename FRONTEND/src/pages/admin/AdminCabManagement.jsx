...
// ── Badge type CAB ───────────────────────────────────────────
const TYPE_COLORS = {
  'URGENT':   { bg: '#fef2f2', color: '#ef4444', border: '#fee2e2', label: 'Urgent (E-CAB)' },
  'NORMAL':   { bg: '#eff6ff', color: '#3b82f6', border: '#dbeafe', label: 'Normal (CAB)' },
};

const TypeBadge = ({ type }) => {
  const s = TYPE_COLORS[type] || { bg: '#f8fafc', color: '#64748b', border: '#e2e8f0', label: type };
  return (
    <span style={{
...
const AdminCabManagement = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setCabs(cabData);
      setUsers(rawUsers.map(u => ({ ...u, _hasRole: (role) => hasRole(u, role) })));
    } catch (e) {
  ...
  useEffect(() => { fetchData(); }, [fetchData]);
    ...
    useEffect(() => {
        ...
        socket.on('cab:vote', handleWs);
        return () => { socket.off('cab:vote', handleWs); };
    }, [fetchData, socket]);
  ...
  const handleEditClick = (cab) => {
    const president = cab.membres?.find(m => m.role === 'PRESIDENT');
    const presidentId = president?.id_user || president?.utilisateur?.id_user || '';
    const memberIds = cab.membres
      ?.filter(m => m.role !== 'PRESIDENT')
      .map(m => m.id_user || m.utilisateur?.id_user)
      .filter(Boolean) || [];
  ...
  const handleEditSubmit = async (e) => {
    ...
    setCreateLoading(true);
    try {
      const currentPresident = editCab.membres?.find(m => m.role === 'PRESIDENT');
      const currentPresidentId = currentPresident?.id_user || currentPresident?.utilisateur?.id_user || '';
      const currentMemberIds = editCab.membres
        ?.filter(m => m.role !== 'PRESIDENT')
        .map(m => m.id_user || m.utilisateur?.id_user)
        .filter(Boolean) || [];
      const membresOps = [];
      ...
      currentMemberIds.forEach(id => {
        if (!editForm.member_ids.includes(id))
          membresOps.push({ action: 'REMOVE', id_user: id });
      });

      const payload = {
        nom_cab: editForm.nom_cab,
        type_cab: editForm.type_cab,
      };
      if (membresOps.length > 0) payload.membres = membresOps;

      await api.put(`/cab/${editCab.id_cab}`, payload);
  ...
  const handleDeleteCab = (cab) => {
    setConfirmDel({
      title:   '⚠️ Supprimer le CAB définitivement',
      message: `Voulez-vous vraiment supprimer le CAB "${cab.nom_cab || cab.code_metier}" ?\n\nCette action supprimera également toutes les réunions, votes, décisions et membres associés. Cette opération est irréversible.`,
      cab,
    });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      // 1. Récupérer toutes les réunions du CAB
      const reunionsRes = await api.get(`/cab/${cab.id_cab}/reunions`).catch(() => null);
      const reunions = reunionsRes?.data?.reunions || reunionsRes?.reunions || [];

      // 2. Pour chaque réunion, supprimer participants + RFCs agenda (votes/décisions n'ont pas de DELETE route directe, mais la suppression de la réunion suffit si elles sont en CASCADE en BDD)
      for (const reunion of reunions) {
        const rid = reunion.id_reunion;

        // Supprimer les participants
        const parts = await api.get(`/reunions/${rid}/participants`).catch(() => null);
        const participants = parts?.data?.participants || parts?.participants || [];
        for (const p of participants) {
          const uid = p.id_user || p.utilisateur?.id_user;
          if (uid) await api.delete(`/reunions/${rid}/participants/${uid}`).catch(() => {});
        }

        // Supprimer les RFCs de l'agenda
        const rfcsRes = await api.get(`/reunions/${rid}/rfcs`).catch(() => null);
        const rfcList = rfcsRes?.data?.rfcs || rfcsRes?.rfcs || [];
        for (const r of rfcList) {
          const rid2 = r.id_rfc || r.rfc?.id_rfc;
          if (rid2) await api.delete(`/reunions/${rid}/rfcs/${rid2}`).catch(() => {});
        }
      }

      // 3. Supprimer les membres du CAB
      const membresRes = await api.get(`/cab/${cab.id_cab}/membres`).catch(() => null);
      const membres = membresRes?.data?.membres || membresRes?.membres || [];
      for (const m of membres) {
        const uid = m.utilisateur?.id_user || m.id_user;
        if (uid) await api.delete(`/cab/${cab.id_cab}/membres/${uid}`).catch(() => {});
      }

      // 4. Supprimer le CAB (maintenant sans réunions ni membres bloquants)
      await api.delete(`/cab/${cab.id_cab}`);

      setToast({ msg: `CAB "${cab.nom_cab}" supprimé avec succès.`, type: 'success' });
      if (detailCab?.id_cab === cab.id_cab) { setDetailCab(null); setShowDetail(false); }
      if (editCab?.id_cab   === cab.id_cab) { setEditCab(null);   setShowEdit(false); }
      await fetchData();
    } catch (err) {
      setCabs(originalCabs);
      setToast({ msg: err?.response?.data?.message || err?.error?.message || err?.message || 'Erreur lors de la suppression', type: 'error' });
    } finally {
      setCreateLoading(false);
    }
  };
  ...
  return (
    <div className="cab-admin-page">
      ...
      <div className="stats-grid">
        ...
        <StatCard
          title="Normal (CAB)"
          value={cabs.filter(c => c.type_cab === 'NORMAL' || c.type_cab === 'STANDARD').length}
          icon={<FiCheckCircle />}
          color="amber"
        ...
        filters={[
          {
            ...
            options: [
              { value: "NORMAL", label: "Normal (CAB)" },
              { value: "URGENT", label: "Urgent (E-CAB)" }
            ]
          }
        ]}
        onReset={() => { setSearch(''); setFilterType('ALL'); }}
        showReset={!!(search || filterType !== 'ALL')}
      />
          ...
          <table style={{ width: '100%', minWidth: '900px', borderCollapse: 'collapse' }}>
            ...
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                {/* Sticky gauche */}
                <th className="sticky-col-first" style={{
                ...
                {/* Sticky droite */}
                <th className="sticky-col-last" style={{
            ...
            <tbody>
              {loading ? (
                ...
                >
                  ...
                  <td className="sticky-col-first" style={{
                  ...
                  <td className="sticky-col-last" style={{
          ...
          <div className="modal-box glass-card" style={{ maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              ...
              <div className="rfc-style-header-text">
                <h2 style={{ color: '#ffffff' }}>Créer un nouveau CAB</h2>
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Définition d'un nouveau comité consultatif de changement</div>
              </div>
              <button className="close-btn-rfc-style" onClick={() => setShowCreate(false)} style={{ color: '#ffffff' }}><FiX size={24} /></button>
            </div>
                  ...
                  <div className="form-group-cab">
                    ...
                    <div className="input-with-icon-cab">
                      ...
                      <select value={createForm.type_cab} onChange={e => setCreateForm({ ...createForm, type_cab: e.target.value })}>
                        ...
                        <option value="URGENT">Urgent (E-CAB)</option>
                      </select>
                    </div>
                  </div>
                </div>
                  ...
                  {!createForm.id_president ? (
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                      ...
                      <select value="" onChange={e => { if (e.target.value) setCreateForm({ ...createForm, id_president: e.target.value }); }} className="modal-select-cab" style={{ flex: 1, padding: '0.5rem', borderColor: '#fde68a' }}>
                        ...
                        {users.filter(u => u._hasRole('CHANGE_MANAGER')).map(u => (
                ...
                <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#f1f5f9', borderRadius: '12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '1rem' }}>
                    ...
                    >
                      ...
                      {users.filter(u => u._hasRole('MEMBRE_CAB')).filter(u => !createForm.member_ids.includes(u.id_user)).map(u => (
                        <option key={u.id_user} value={u.id_user}>{u.prenom_user} {u.nom_user}</option>
                      ))}
                    </select>
                  </div>
        ...
        <div className="modal-backdrop" onClick={() => setShowDetail(false)}>
          <div className="modal-box glass-card" style={{ maxWidth: '700px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              ...
              <div className="rfc-style-header-text">
                <h2 style={{ color: '#ffffff' }}>Détails du CAB</h2>
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>#{detailCab.code_metier} — {detailCab.nom_cab}</div>
              </div>
              <div className="rfc-style-actions">
                <button className="rfc-action-btn edit" onClick={() => handleEditClick(detailCab)} style={{ border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.1)', color: 'white' }}><FiEdit3 /> Modifier</button>
                <button className="rfc-action-btn delete" onClick={() => handleDeleteCab(detailCab)} style={{ border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.1)', color: '#fecaca' }}><FiTrash2 /> Supprimer</button>
              </div>
              <button className="close-btn-rfc-style close-btn-offset" onClick={() => setShowDetail(false)} style={{ color: '#ffffff' }}><FiX size={24} /></button>
            </div>
            ...
            <div className="modal-body-rfc-style">
              <div className="rfc-style-grid">
                ...
                <div className="detail-item-rfc-style">
                  ...
                  <div className="detail-value-box" style={{ background: '#fef3c7', borderColor: '#fde68a', color: '#92400e' }}>
                    {(() => {
                      const p = detailCab.membres?.find(m => m.role === 'PRESIDENT')?.utilisateur || detailCab.president;
                      return p
                        ? <><Avatar prenom={p.prenom_user} nom={p.nom_user} size={24} radius="6px" /> <span style={{ fontWeight: 600 }}>{p.prenom_user} {p.nom_user}</span></>
                        : 'Non assigné';
                    })()}
                  </div>
                </div>
      ...
      {showEdit && (
        <div className="modal-backdrop" onClick={() => setShowEdit(false)}>
          <div className="modal-box glass-card" style={{ maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              <div className="rfc-style-icon-wrapper edit" style={{ background: 'rgba(255, 255, 255, 0.1)', color: '#ffffff', borderColor: 'rgba(255, 255, 255, 0.2)' }}><FiEdit3 /></div>
              <div className="rfc-style-header-text">
                <h2 style={{ color: '#ffffff' }}>Modifier le CAB</h2>
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Mise à jour des paramètres du comité</div>
              </div>
              <button className="close-btn-rfc-style" onClick={() => setShowEdit(false)} style={{ color: '#ffffff' }}><FiX size={24} /></button>
            </div>
        ...
        <div className="modal-backdrop" onClick={() => setShowMembersList(false)}>
          <div className="modal-box glass-card" style={{ maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              ...
              <div className="rfc-style-header-text">
                <h2 style={{ color: '#ffffff' }}>Membres du Comité</h2>
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>{selectedCabForMembers.nom_cab || 'Comité de Changement'}</div>
              </div>
              <button className="close-btn-rfc-style" onClick={() => setShowMembersList(false)} style={{ color: '#ffffff' }}><FiX size={24} /></button>
            </div>
