...
import RfcProcessModal from '../changemanager/components/RfcProcessModal';
...
const AdminRfcManagement = () => {
  ...
  const handlePreevaluer = async () => {
    ...
    try {
      ...
      showToast('RFC pré-aprouver et transférée avec succès !', 'success');
        ...
        filters={[
          {
            ...
            options: sortedStatuses.map(s => ({
              value: s.code_statut,
              label: s.code_statut === 'PRE_APPROUVEE' ? 'Pré-aprouver' : s.code_statut === 'EVALUEE' ? 'Evaluer' : s.libelle
            }))
          },
          {
            value: filterType,
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', minWidth: '1100px', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>Chargement...</td>
                </tr>
              ) : paginated.length === 0 ? (
                <tr>
