    ...
    <div className="audit-page">
                ...
                <tr key={log.id_log} onClick={() => handleRowClick(log)} className="audit-row" style={{ cursor: 'pointer' }}>
                  <td className="td-user sticky-col-first">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <Avatar 
                        prenom={log.utilisateur?.prenom_user || ''} 
                        nom={log.utilisateur?.nom_user || ''} 
                        size={28} 
                      />
                      <span>
                        {log.utilisateur?.prenom_user || log.utilisateur?.nom_user 
                          ? `${log.utilisateur.prenom_user || ''} ${log.utilisateur.nom_user || ''}`.trim() 
                          : 'Système'}
                      </span>
                    </div>
                  </td>
      ...
      {isModalOpen && selectedLog && (
        <div className="modal-backdrop-cab" onClick={closeModal}>
          <div className="modal-box-cab glass-card-cab audit-detail-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px', background: '#f0f9ff', border: '1px solid #003366' }}>
            <div className="modal-top-rfc-style" style={{ background: '#003366', borderBottom: '1px solid #002855', padding: '1.5rem 2rem' }}>
              ...
              <div className="rfc-style-header-text">
                <h2 style={{ color: '#ffffff' }}>Détails du Journal d'Audit</h2>
                <div className="rfc-style-subtitle" style={{ color: 'rgba(255, 255, 255, 0.7)' }}>Historique et trace des actions</div>
              </div>
              <button className="close-btn-rfc-style" onClick={closeModal} style={{ color: '#ffffff' }}><FiX size={24} /></button>
            </div>
            ...
            <div className="modal-body-rfc-style">
              <div className="audit-detail-grid">
                ...
                <div className="detail-item">
                  ...
                  <div className="user-info-detail" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <Avatar 
                      prenom={selectedLog.utilisateur?.prenom_user || ''} 
                      nom={selectedLog.utilisateur?.nom_user || ''} 
                      size={40} 
                    />
                    <div>
                      <p className="user-name" style={{ margin: 0, fontWeight: 600 }}>
                        {selectedLog.utilisateur?.prenom_user || selectedLog.utilisateur?.nom_user 
                          ? `${selectedLog.utilisateur.prenom_user || ''} ${selectedLog.utilisateur.nom_user || ''}`.trim() 
                          : 'Système'}
                      </p>
                      {selectedLog.utilisateur?.email_user && (
                        <p className="user-email" style={{ margin: 0, fontSize: '0.8rem', color: '#64748b' }}>
                          {selectedLog.utilisateur.email_user}
                        </p>
                      )}
                    </div>
      ...
      {reportModal.open && (
        <div className="modal-backdrop-cab" onClick={() => setReportModal({ open: false, data: null, loading: false })}>
          ...
          style={{ width: '1200px', maxWidth: '95vw', maxHeight: '90vh', position: 'relative', display: 'flex', flexDirection: 'column' }}
        ...
        >
          <div className="premium-modal-blue-header no-print">
            ...
            <div className="modal-body-rfc-style report-print-area" style={{ flex: 1, overflowY: 'auto' }}>
              {reportModal.loading ? (
                <div className="loading-state">
                  <FiRefreshCw className="spinning" />
