  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      for (const cab of allCabs) {
        const isMember = cab.membres?.some(
          m => m.id_user === currentUser.id_user || m.utilisateur?.id_user === currentUser.id_user
        );
  ...
  return (
    <div className="cm-page">
      {voteResultModal && (
        <div onClick={() => setVoteResultModal(null)} className="cab-modal-overlay">
          <div onClick={e => e.stopPropagation()} className="cab-modal" style={{ maxWidth: '380px', textAlign: 'center', padding: '2rem' }}>
            <div style={{ fontSize: '2.5rem', marginBottom: '1.5rem' }}>{voteResultModal.type === 'success' ? '✅' : '❌'}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
        {[
          { label: 'Mes Sessions', value: meetings.length, filter: '', colorClass: 'blue', icon: <FiCalendar size={24} /> },
          { label: 'À venir', value: meetings.filter(m => new Date(m.date_reunion) > new Date()).length, filter: 'UPCOMING', colorClass: 'purple', icon: <FiClock size={24} /> },
          { label: 'Urgentes', value: meetings.filter(m => ['URGENT','URGENCE'].includes(m.cab_type?.toUpperCase())).length, filter: 'URGENT', colorClass: 'red', icon: <FiAlertTriangle size={24} /> },
        ].map(k => (
          ...
          {selectedMeeting ? (
            <div className="cm-workspace-inner">
              ...
              <div className="cm-tabs">
                <button className={`cm-tab-btn ${activeTab === 'rfcs_votes' ? 'active' : ''}`} onClick={() => setActiveTab('rfcs_votes')}><FiTrendingUp /> Changement & Vote</button>
                ...
                {activeTab === 'rfcs_votes' && (
                  <div className="cm-votes-view">
                    {rfcs.map(rfc => {
                      ...
                      // Highlight the President in the votes detail if it existed here
                      // But the user asked for "cadre de president" in yellow. 
                      // Let's add a small section below each card showing the President's status if they voted.
                      const president = selectedMeeting.cab_membres?.find(m => m.role === 'PRESIDENT');
                      const presidentVote = rfcVotes.find(v => (v.utilisateur?.id_user || v.id_user) === (president?.id_user || president?.utilisateur?.id_user));
                        ...
                        <div key={rfc.id_rfc} className={`cm-rfc-vote-card ${isClosed ? 'closed' : ''}`} onClick={() => handleEvaluationClick(rfc)} style={{ cursor: 'pointer', position: 'relative' }}>
                          ...
                          <div className="cm-rvc-header">
                            ...
                            <div className="cm-rvc-stats-bar">
                              <div className="cm-rvc-stat-item app"><span>{rfcVotes.filter(v => v.valeur_vote === 'APPROUVER').length}</span> Pour</div>
                              <div className="cm-rvc-stat-item rej"><span>{rfcVotes.filter(v => v.valeur_vote === 'REJETER').length}</span> Contre</div>
                            </div>
                            {!isClosed && !myVote && <div style={{ color: '#6366f1', fontSize: '0.85rem', fontWeight: 600, marginTop: '1rem', textAlign: 'right' }}><FiEdit3 /> Évaluer et voter</div>}
                          </div>
                      );
                    })}
                  </div>
      ...
      {showRecModal && currentRecs && (
        <div className="cab-modal-overlay" onClick={() => setShowRecModal(false)}>
          <div className="cab-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            <div className="cab-modal-header">
              <div className="header-with-icon">
                <div className="header-icon-circle" style={{ background: currentRecs.empty ? '#fff1f2' : '#eff6ff', color: currentRecs.empty ? '#e11d48' : '#3b82f6' }}>
              ...
              <button className="modal-close" onClick={() => setShowRecModal(false)}><FiX /></button>
            </div>
            <div className="cab-modal-body">
              {currentRecs.empty ? (
                ...
                <div className="recommendations-grid">
                  <div className="rec-item" style={{ gridColumn: '1 / -1', background: '#f8fafc', border: '1.5px solid #e2e8f0' }}>
                    <div className="rec-label" style={{ color: '#1e293b' }}><FiAlertTriangle /> Niveau de Risque</div>
                  ...
                  <div className="rec-item">
                    <div className="rec-label"><FiCheckCircle /> Conditions d'exécution</div>
                    <div className="rec-value">{currentRecs.conditions}</div>
                  </div>
                  <div className="rec-item">
                    <div className="rec-label"><FiShield /> Sécurité / Rollback</div>
                    <div className="rec-value">{currentRecs.securite_rollback}</div>
                  </div>
                  <div className="rec-item">
                    <div className="rec-label"><FiAlertTriangle /> Précautions</div>
                    <div className="rec-value">{currentRecs.precautions}</div>
                  </div>
                </div>
              )}
            </div>
            <div className="cab-modal-footer">
              <button className="btn-close-rec" onClick={() => setShowRecModal(false)}>Fermer</button>
            </div>
          </div>
        </div>
      )}
