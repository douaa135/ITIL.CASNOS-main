...
const RfcEvaluationList = () => {
  ...
  useEffect(() => {
    ...
    const fetchRfcs = async () => {
        try {
            setLoading(true);
            const allRfcs = await rfcService.getAllRfcs();
            console.log('All RFCs fetched:', allRfcs);
            
            // Temporary: Show ALL RFCs to debug why the list is empty
            setRfcs(allRfcs || []);
        } catch (error) {
        ...
        <div className="card-content">
          {loading ? (
            ...
            <>
                      ...
                      <tr key={rfc.id_rfc} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '1rem' }}>
                          <span style={{ padding: '4px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '600', backgroundColor: '#e0e7ff', color: '#4f46e5' }}>
                            {rfc.typeRfc?.type || rfc.type?.nom_type || 'N/A'}
                          </span>
                        </td>
                        <td style={{ padding: '1rem' }}>
                           <span className={`rfc-priority priority-${(rfc.priorite?.libelle || rfc.priorite || 'basse').toLowerCase()}`}>
                            {rfc.priorite?.libelle || rfc.priorite || '—'}
                          </span>
                        </td>
                        <td style={{ padding: '1rem' }}>
                          <span style={{ padding: '4px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '600', backgroundColor: '#f1f5f9', color: '#475569' }}>
                            {rfc.statut?.libelle || rfc.statut?.code_statut}
                          </span>
                        </td>
              ...
              {/* Debug info */}
              <div style={{ padding: '1rem', fontSize: '0.7rem', color: '#94a3b8', textAlign: 'right' }}>
                DEBUG: {rfcs.length} RFCs au total après fetch.
              </div>
            </>
          )}
        </div>
