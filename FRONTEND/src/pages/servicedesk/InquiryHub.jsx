...
const InquiryHub = () => {
  ...
  const handleTriageDecision = async (statusCode) => {
    ...
    if (statusCode === 'PRE_APPROUVEE' && (!selectedType || !selectedEnv)) {
      showToast('Vous devez classifier le Type et assigner l\'Environnement avant d\'accepter.', 'warning');
      return;
    }
    ...
    try {
      ...
      await rfcService.updateRfcStatus(selectedItem.id_rfc, targetStatus.id_statut, {
        id_env: statusCode === 'PRE_APPROUVEE' ? selectedEnv : undefined,
            ...
            <tbody>
              {loading ? (
                ...
                >
                  ...
                  {/* 5. Env */}
                  <td style={tdStyle}>
                    <EnvironmentBadge item={rfc} environments={environnements} />
                  </td>
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()} style={{ width: '750px', maxWidth: '90vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="modal-body" style={{ overflowY: 'auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.75rem', fontWeight: '700', color: '#475569', textTransform: 'uppercase' }}>Environnement Cible <span style={{ color: '#ef4444' }}>*</span></label>
                  <select value={selectedEnv} onChange={e => setSelectedEnv(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: '#f8fafc', fontSize: '0.9rem', outline: 'none', color: '#334155' }}>
