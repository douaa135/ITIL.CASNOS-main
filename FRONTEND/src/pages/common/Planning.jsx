...
const Planning = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const resAllBlackouts = await planningService.getAllBlackouts().catch(() => ({ data: { blackouts: [] } }));
  ...
  const handleCreateBlackout = async (e) => {
    ...
    try {
      if (selectedBlackout) {
        await planningService.updateBlackout(selectedBlackout.id_blackout, blackoutForm);
        setToast({ msg: 'Période blackout mise à jour', type: 'success' });
      } else {
        await planningService.createBlackout(blackoutForm);
    ...
    return (
      <div className="calendar-grid">
        ...
        {cells.map((cell, idx) => {
          ...
          const iso = cell.date ? cell.date.toISOString().split('T')[0] : null;
          
          const dayChanges = iso ? data.changements.filter(c => {
            const start = c.date_debut?.split('T')[0];
            const end = c.date_fin_prevu?.split('T')[0] || start;
            return iso >= start && iso <= end;
          }) : [];

          const dayBlackouts = iso ? data.blackouts.filter(b => {
            const start = b.date_debut?.split('T')[0];
            const end = b.date_fin?.split('T')[0];
            if (b.recurrent) {
          ...
          return (
            <div key={idx} className={`calendar-day-cell ${cell.type === 'other' ? 'other-month' : ''} ${isToday ? 'today' : ''} ${isWE ? 'weekend' : ''}`}>
              ...
              {dayChanges.map(c => (
                <div key={c.id_changement} className={`calendar-event event-change ${c.statut?.code_statut}`} title={c.rfc?.titre_rfc}>
                  #{c.code_changement} {c.rfc?.titre_rfc}
                </div>
              ))}
            ...
            <div key={offset} className="mini-month">
              ...
              <div className="mini-grid">
                ...
                {days.map((d, i) => {
                  ...
                  const hasChange = monthChanges.some(c => c.date_debut?.startsWith(iso));
                  ...
                  const isMiniWE = (i % 7 === 5 || i % 7 === 6); // V, S (getDay offset in grid)
                  // Wait! The grid starts on Lundi (idx 0). So V is idx 4, S is idx 5.
                  ...
                  return (
                    <div 
                      ...
                      style={hasBlackout ? { background: '#ef4444', color: '#ffffff' } : {}}
                    >
                      {hasBlackout ? d : (!isWeekendMini ? d : <span style={{ fontSize: '0.5rem', opacity: 0.5 }}>WE</span>)}
                    </div>
                  );
                })}
              </div>
